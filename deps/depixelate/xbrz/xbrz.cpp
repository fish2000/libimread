...
namespace {
    ...
    void alphaBlend(uint32_t& dst, uint32_t col) {
        ...
        dst = (redMask & ((col & redMask) * N + (dst & redMask) * (M - N)) /
                             M) | // this works because 8 upper bits are free
    ...
    inline uint32_t alphaBlend2(uint32_t pix1, uint32_t pix2, double alpha) {
        return (redMask &
               ...
               (greenMask & static_cast<uint32_t>((pix1 & greenMask) * alpha +
                                                  (pix2 & greenMask) * (1 - alpha))) |
               (blueMask &
...
#define FORCE_INLINE inline
    ...
    */
    ...
    double distHSL(uint32_t pix1, uint32_t pix2, double lightningWeight)
    {
        ...
        //however we interpret the cylinder as a bi-conic solid with top/bottom radius 0, middle
    radius 1 assert(0 <= h1 && h1 <= 1); assert(0 <= h2 && h2 <= 1);
    ...
    */
    ...
    inline double distNonLinearRGB(uint32_t pix1, uint32_t pix2) {
        ...
        return sqrt((2 + r_avg / 255) * square(r_diff) + 4 * square(g_diff) +
                    (2 + (255 - r_avg) / 255) * square(b_diff));
    }
    ...
    inline double distYUV(uint32_t pix1, uint32_t pix2, double luminanceWeight) {
        ...
        // since RGB -> YUV conversion is essentially a matrix multiplication, we can calculate the
        // RGB diff before the conversion (distributive property)
        const double r_diff = static_cast<int>(getRed(pix1)) - getRed(pix2);
        const double g_diff = static_cast<int>(getGreen(pix1)) - getGreen(pix2);
        const double b_diff = static_cast<int>(getBlue(pix1)) - getBlue(pix2);
    ...
    inline double colorDist(uint32_t pix1, uint32_t pix2, double luminanceWeight) {
    ...
    */
            ...
            const xbrz::ScalerCfg& cfg) // result: F, G, J, K corners of "GradientType"
    {
        ...
        auto dist = [&](uint32_t col1, uint32_t col2) {
...
#define DEF_GETTER(x)                                                                              \
    ...
    // we cannot and NEED NOT write "ker.##x" since ## concatenates preprocessor tokens but "." is
    // not a token
    DEF_GETTER(a)
    DEF_GETTER(b) DEF_GETTER(c) DEF_GETTER(d) DEF_GETTER(e) DEF_GETTER(f) DEF_GETTER(g)
        DEF_GETTER(h) DEF_GETTER(i)
#undef DEF_GETTER
...
#define DEF_GETTER(x, y)                                                                           \
    ...
    inline uint32_t get_##x<ROT_180>(const Kernel_3x3& ker) {                                      \
        return ker.y;                                                                              \
                    ...
                    DEF_GETTER(a, i) DEF_GETTER(b, h) DEF_GETTER(c, g) DEF_GETTER(d, f)
                        DEF_GETTER(e, e) DEF_GETTER(f, d) DEF_GETTER(g, c) DEF_GETTER(h, b)
                            DEF_GETTER(i, a)
#undef DEF_GETTER
    ...
    inline uint32_t get_##x<ROT_270>(const Kernel_3x3& ker) {                                      \
        return ker.y;                                                                              \
                                ...
                                DEF_GETTER(a, c) DEF_GETTER(b, f) DEF_GETTER(c, i) DEF_GETTER(d, b)
                                    DEF_GETTER(e, e) DEF_GETTER(f, h) DEF_GETTER(g, a)
                                        DEF_GETTER(h, d) DEF_GETTER(i, g)
#undef DEF_GETTER
    ...
    */
    ...
    FORCE_INLINE // perf: quite worth it!
        void
        scalePixel(const Kernel_3x3& ker, uint32_t* target, int trgWidth,
...
#define i get_i<rotDeg>(ker)
        ...
        if (breakIntoDebugger) {
            ...
            debug_break(); /// q.v. debugbreak.h supra.
        }
#endif
        ...
        if (getBottomR(blend) >= BLEND_NORMAL) {
            auto eq = [&](uint32_t col1, uint32_t col2) {
            ...
            if (doLineBlend) {
                const double fg = dist(f, g); // test sample: 70% of values max(fg, hc) / min(fg,
                                              // hc) are between 1.1 and 3.7 with median being 1.9
...
#undef i
    ...
    void scaleImage(const uint32_t* src, uint32_t* trg, int srcWidth, int srcHeight,
        ...
        //even could use larger area of "sizeof(uint32_t) * srcWidth * (yLast - yFirst)" bytes
        //without risk of accidental overwriting before accessing
        ...
        // initialize preprocessing buffer for first row: detect upper left and right corner
        // blending  this cannot be optimized for adjacent processing stripes; we must not allow for a
        ...
        for (int y = yFirst; y < yLast; ++y) {
            ...
            for (int x = 0; x < srcWidth; ++x, out += Scaler::scale) {
                ...
                fillBlock(out, trgWidth * sizeof(uint32_t), s_0[x],
...
bool xbrz::equalColor(uint32_t col1, uint32_t col2, double luminanceWeight,
...
void xbrz::nearestNeighborScale(const uint32_t* src, int srcWidth, int srcHeight, int srcPitch,
    ...
    switch (st) {
        case NN_SCALE_SLICE_SOURCE:
            // nearest-neighbor (going over source image - fast for upscaling, since source is read
            // only once
            yFirst = std::max(yFirst, 0);
            yLast  = std::min(yLast, srcHeight);
