...
static bool scale_hqx(const void* srcData, void* dstData, int width, int height, int scale) {
    hqxInit();
    switch (scale) {
        case 2: hq2x_32((uint32_t*)srcData, (uint32_t*)dstData, width, height);
        case 3: hq3x_32((uint32_t*)srcData, (uint32_t*)dstData, width, height);
        case 4: hq4x_32((uint32_t*)srcData, (uint32_t*)dstData, width, height);
    }
    return true;
}
...
static const Algorithm algorithms[] = {
    { "xbrz", 2, 5, scale_xbrz }, { "hqx", 2, 4, scale_hqx }, { "scale2x", 2, 4, scale_scale2x },
};
...
int depixelate_main(int argc, const char* argv[]) {
    @autoreleasepool {
        ...
        if (!algorithm->scaleFunction(srcData, dstData, (int)rep.pixelsWide, (int)rep.pixelsHigh,
                                      scale)) {
        ...
        NSBitmapImageRep* dstRep =
            [[NSBitmapImageRep alloc] initWithBitmapDataPlanes:(unsigned char**)&dstData
        ...
        NSData* finalData = [dstRep representationUsingType:NSPNGFileType properties:@{}];
        ;
