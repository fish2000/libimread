...
namespace inicpp {
    ...
    namespace string_utils {
        ...
        std::string right_trim(std::string const& str) {
            auto back =
                std::find_if(str.rbegin(), str.rend(), [](int c) { return !std::isspace(c); })
                    .base();
        ...
        boolean_ini_t parse_string<boolean_ini_t>(std::string const& value,
            ...
            if (value == "0" || value == "f" || value == "n" || value == "off" || value == "no" ||
                value == "disabled") {
                return false;
            } else if (value == "1" || value == "t" || value == "y" || value == "on" ||
                       value == "yes" || value == "enabled") {
                return true;
            } else {
                throw invalid_type_exception("Option '" + option_name +
        ...
        template <>
        enum_ini_t parse_string<enum_ini_t>(std::string const& value, std::string const&) {
    ...
    namespace inistd {
        ...
        std::string to_string(enum_ini_t const& value) { return static_cast<std::string>(value); }
