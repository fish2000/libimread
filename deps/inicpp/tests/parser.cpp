...
TEST(inicpp::parser, load_config) {
    EXPECT_THROW(inicpp::parser::load_file("nonexisting_file.txt"), inicpp::parser_exception);
    ...
    auto loaded_config = inicpp::parser::load(str_config);
    ...
    EXPECT_EQ(loaded_config[0][0].get<inicpp::string_ini_t>(), "val");
    std::vector<std::string> expected_list{ "val2", "val3", "val4" };
    EXPECT_EQ(loaded_config[0][1].get_list<inicpp::string_ini_t>(), expected_list);
    EXPECT_EQ(loaded_config[1][0].get<inicpp::string_ini_t>(), "val");
    
    str_config = ""
                 "[section]\n"
                 "opt = 15";
    inicpp::schema validatin_schema;
    
    // Following is not possible in MS Visual C++ yet
    // inicpp::section_schema_params sect_params{ "section", inicpp::item_requirement::mandatory, "comment" };
    inicpp::section_schema_params sect_params;
    sect_params.name        = "section";
    sect_params.requirement = inicpp::item_requirement::mandatory;
    sect_params.comment     = "comment";
    inicpp::section_schema sect_schema(sect_params);
    
    inicpp::option_schema_params<inicpp::unsigned_ini_t> opt_params;
    opt_params.name        = "opt";
    opt_params.requirement = inicpp::item_requirement::mandatory;
    opt_params.type        = inicpp::option_item::single;
    sect_schema.add_option(opt_params);
    validatin_schema.add_section(sect_schema);
    
    inicpp::config my_config;
    EXPECT_NO_THROW(my_config = inicpp::parser::load(str_config, validatin_schema, inicpp::schema_mode::strict));
    EXPECT_EQ(my_config["section"]["opt"].get<inicpp::unsigned_ini_t>(), 15u);
    
    opt_params.name = "other name";
    inicpp::option_schema opt_schema(opt_params);
    validatin_schema.add_option("section", opt_schema);
    EXPECT_THROW(inicpp::parser::load(str_config, validatin_schema, inicpp::schema_mode::strict),
                 inicpp::validation_exception);
}
...
TEST(parser, store_config) {
    ...
    // Following is not possible in MS Visual C++ yet
    // inicpp::section_schema sect_schema({ "section_name", inicpp::item_requirement::mandatory, "comment" });
    ...
    // inicpp::option_schema_params<inicpp::string_ini_t> opt { {"opt", inicpp::item_requirement::mandatory,
    //  inicpp::option_item::single, "default value", "opt comment"}, nullptr};
    
    inicpp::option_schema_params<inicpp::string_ini_t> opt1;
    opt1.name          = "opt";
    opt1.requirement   = inicpp::item_requirement::mandatory;
    opt1.type          = inicpp::option_item::single;
    opt1.default_value = "default value";
    opt1.comment       = "opt comment";
    opt1.validator     = nullptr;
    sect_schema.add_option(opt1);
    
    inicpp::option_schema_params<inicpp::unsigned_ini_t> opt2;
    opt2.name          = "unsigned";
    opt2.requirement   = inicpp::item_requirement::optional;
    opt2.type          = inicpp::option_item::single;
