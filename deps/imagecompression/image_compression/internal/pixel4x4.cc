...
namespace image_codec_compression {
    ...
    void Pixel4x4::ConstructOutsideImage(const ColorType* pixels,
        ...
        for (int y = 0; y < 4; ++y) {
            ...
            const ColorType* row_pixels = GetImagePixel(pixels, bytes_per_row, std::min(row + y, height - 1), 0);
            // Regions 1 and 3:
    ...
    template
    void Pixel4x4::ConstructOutsideImage<Rgb888>(const Rgb888* pixels,
                                                 uint32_t image_height, uint32_t image_width,
                                                 uint32_t bytes_per_row,
                                                 int row, int column);
    
    template
    void Pixel4x4::ConstructOutsideImage<Rgba8888>(const Rgba8888* pixels,
                                                   uint32_t image_height, uint32_t image_width,
                                                   uint32_t bytes_per_row,
                                                   int row, int column);
