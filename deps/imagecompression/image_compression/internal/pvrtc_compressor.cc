...
namespace imagecompression {
    ...
    //   3 = color1
    static Rgba8888 ApplyModulation(Rgba8888 color0, Rgba8888 color1, uint32_t mod) {
    ...
    static Rgba8888 Interpolate4_2BPP(Rgba8888 color00, Rgba8888 color01, Rgba8888 color10,
                                      Rgba8888 color11, uint32_t px, uint32_t py) {
        // Calculate the weights that should be applied to the four input colors.
        const uint32_t a = (kBlockHeight - py) * (kBlockWidth - px);
        const uint32_t b = (kBlockHeight - py) * px;
        const uint32_t c = py * (kBlockWidth - px);
        const uint32_t d = py * px;
        // Apply these weights.
        const uint32_t downscale = kBlockWidth * kBlockHeight;
    ...
    static Rgba8888 GetInterpolatedColor2BPP(const Rgba8888* source,
        ...
        const uint32_t source_left = ((x - kBlockWidth / 2) & (width - 1)) >> kLog2BlockWidth;
        const uint32_t source_top = ((y - kBlockHeight / 2) & (height - 1)) >> kLog2BlockHeight;
        const uint32_t source_right = (source_left + 1) & ((width >> kLog2BlockWidth) - 1);
        const uint32_t source_bottom = (source_top + 1) & ((height >> kLog2BlockHeight) - 1);
        // The bilinear weights to be used for interpolation.
        const uint32_t x_weight = (x + kBlockWidth / 2) & (kBlockWidth - 1);
        const uint32_t y_weight = (y + kBlockHeight / 2) & (kBlockHeight - 1);
    ...
    static void GetExtremesFast(const Rgba8888* image,
        ...
        for (uint32_t i = 0; i < PAIRS; i++) {
            ...
            best_fitness[i][0] = static_cast<uint32_t>(-1);
            best_fitness[i][1] = 0;
            best_index[i][0]   = 0;
            best_index[i][1]   = 0;
        }
        ...
        for (uint32_t y = y0; y < y0 + kBlockHeight; y++) {
            for (uint32_t x = x0; x < x0 + kBlockWidth; x++) {
                uint32_t x_wrapped = (x + width) & (width - 1);
                uint32_t y_wrapped = (y + height) & (height - 1);
        ...
        for (uint32_t i = 0; i < PAIRS; i++) {
            uint32_t diff = ColorDiff(image[best_index[i][0]], image[best_index[i][1]]);
    ...
    uint32_t CalculateBlockModulationData(const uint8_t* image_mod,
                                          uint32_t width,   uint32_t height,
                                          uint32_t block_x, uint32_t block_y,
                                          ModulationMode mode) {
        uint32_t result = 0;
    ...
    static void Morph(const Rgba8888* image, uint32_t width, uint32_t height, Rgba8888* outa,
                      Rgba8888* outb) {
        for (uint32_t y = 0; y < height; y += kBlockHeight) {
            for (uint32_t x = 0; x < width; x += kBlockWidth) {
                uint32_t indexa = 0;
                uint32_t indexb = 0;
    ...
    // to be pre-allocated.
    static void Modulate(const Rgba8888* image, uint32_t width, uint32_t height, const Rgba8888* imagea,
                         const Rgba8888* imageb, unsigned char* mod) {
        for (uint32_t y = 0; y < height; y++) {
            for (uint32_t x = 0; x < width; x++) {
                Rgba8888 colora = GetInterpolatedColor2BPP(&imagea[0], width, height, x, y);
                Rgba8888 colorb = GetInterpolatedColor2BPP(&imageb[0], width, height, x, y);
    ...
    static void Encode(uint32_t width, uint32_t height, const Rgba8888* imagea, const Rgba8888* imageb,
                       const uint8_t* image_mod, uint8_t* pvr) {
        // Loop through all output blocks.
        for (uint32_t i = 0; i < width * height / (kBlockWidth * kBlockHeight); i++) {
    ...
    static void CompressPVRTC_RGBA_2BPP(const Rgba8888* image,
                                        uint32_t width, uint32_t height,
                                        uint8_t* pvr) {
        uint32_t                low_image_size = (width * height) / (kBlockWidth * kBlockHeight);
        std::vector<Rgba8888>   imagea(low_image_size);
        std::vector<Rgba8888>   imageb(low_image_size);
        std::vector<uint8_t>    imagemod(width * height);
