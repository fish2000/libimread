...
namespace imagecompression {
    ...
    // significant bits.
    inline uint32_t GetMask(int num_ones) { return (1 << num_ones) - 1; }
    ...
    inline int GetBits(uint32_t bits, int start_bit, int num_bits) {
    ...
    // 0 (LSB) to 31 (MSB).
    inline void SetBits(int start_bit, int num_bits, int value, uint32_t* bits) {
        ...
        const uint32_t mask           = GetMask(num_bits);
        const uint32_t unsigned_value = static_cast<uint32_t>(value) & mask;
    ...
    inline int32_t ExtendSignBit(int32_t value, int num_bits) {
