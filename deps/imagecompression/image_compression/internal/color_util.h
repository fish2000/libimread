...
namespace imagecompression {
    ...
    inline const RgbInt operator+(RgbInt const& c0, RgbInt const& c1) {
        return RgbInt(c0.r + c1.r, c0.g + c1.g, c0.b + c1.b);
    }
    
    inline const RgbaInt operator+(RgbaInt const& c0, RgbaInt const& c1) {
    ...
    inline const RgbInt operator/(RgbInt const& c, int32_t div) {
        return RgbInt(c.r / div, c.g / div, c.b / div);
    }
    
    inline const RgbaInt operator/(RgbaInt const& c, int32_t div) {
    ...
    template <typename ColorType> inline
    const RgbInt ToRgbInt(ColorType const& c) {
        return RgbInt(c.r, c.g, c.b);
    }
    
    // Converts an Rgba8888 color to an RgbaInt color.
    inline const RgbaInt ToRgbaInt(Rgba8888 const& c) { return RgbaInt(c.r, c.g, c.b, c.a); }
    
    // Converts an RgbInt color in which each component is assumed to be
    // 0-255 to an Rgb888.
    inline const Rgb888 ToRgb888(RgbInt const& c) { return Rgb888(c.r, c.g, c.b); }
    
    // Converts an RgbaInt color in which each component is assumed to be
    // 0-255 to an Rgba8888.
    inline const Rgba8888 ToRgba8888(RgbaInt const& c) { return Rgba8888(c.r, c.g, c.b, c.a); }
    
    // Converts an Rgb565 to a uint16.
    inline uint16_t ToUInt16(const Rgb565& c) { return (c.r << 11 | c.g << 5 | c.b); }
    
    // Converts a uint16 to an Rgb565.
    inline const Rgb565 ToRgb565(const uint16_t p) {
        return Rgb565(p >> 11, (p >> 5) & 0x3F, p & 0x1F);
    }
    ...
    inline const RgbInt ToBgrInt(RgbInt const& c) { return RgbInt(c.b, c.g, c.r); }
    ...
    const uint8_t Quantize8Fast(int v) {
    ...
    template <int num_bits> inline
    const RgbInt QuantizeRgbFast(RgbInt const& c) {
    ...
    template <int num_bits> inline
    const RgbaInt QuantizeRgbaFast(RgbaInt const& c) {
    ...
    // 0-255 to an Rgb565.
    inline const Rgb565 QuantizeToRgb565(RgbInt const& c) {
    ...
    inline int Extend4Bit(int bits) { return (bits << 4) | bits; }
    ...
    inline int Extend5Bit(int bits) { return (bits << 3) | ((bits >> 2) & 7); }
    ...
    inline const RgbInt ExtendToRgbInt(const Rgb565& c) {
        return RgbInt((c.r << 3) | (c.r >> 2), (c.g << 2) | (c.g >> 4), (c.b << 3) | (c.b >> 2));
    }
    
    // Extends an Rgb565 to an Rgb888.
    inline const Rgb888 ExtendToRgb888(const Rgb565& c) {
        return Rgb888((c.r << 3) | (c.r >> 2), (c.g << 2) | (c.g >> 4), (c.b << 3) | (c.b >> 2));
    }
    ...
    inline int ClampTo8Bits(int value) {
        ...
        const int zero_if_negative = 1 - ((static_cast<uint32_t>(value) & 0x80000000) >> 31);
        return zero_if_negative * std::min(255, value);
    }
    
    // Clamps all components of an RgbInt to 0-255 and returns the result.
    inline const RgbInt ClampRgbInt(RgbInt const& c) {
        return RgbInt(ClampTo8Bits(c.r),
                      ClampTo8Bits(c.g),
                      ClampTo8Bits(c.b));
    }
    
    // Clamps all components of an RgbaInt to 0-255 and returns the result.
    inline const RgbaInt ClampRgbaInt(RgbaInt const& c) {
        return RgbaInt(ClampTo8Bits(c.r),
                       ClampTo8Bits(c.g),
                       ClampTo8Bits(c.b),
                       ClampTo8Bits(c.a));
    }
    ...
    template <int scale0, int scale1> inline
    int CombineIntFast(int value0, int value1) {
    ...
    // overflow.
    template <int scale0, int scale1> inline
    uint8_t CombineUint8Fast(uint8_t value0, uint8_t value1) {
        return static_cast<uint8_t>(CombineIntFast<scale0, scale1>(value0, value1));
    }
    
    // Version of CombineIntFast() that operates on Rgb888 colors.
    template <int scale0, int scale1> inline
    const Rgb888 CombineRgb888Fast(Rgb888 const& color0, Rgb888 const& color1) {
    ...
    template <int scale0, int scale1> inline
    const Rgba8888 CombineRgba8888Fast(Rgba8888 const& color0, Rgba8888 const& color1) {
    ...
    template <int scale0, int scale1> inline
    const RgbInt CombineRgbIntFast(RgbInt const& color0, RgbInt const& color1) {
    ...
    template <int scale0, int scale1> inline
    const RgbaInt CombineRgbaIntFast(RgbaInt const& color0, RgbaInt const& color1) {
    ...
    inline uint8_t Average4Uint8Fast(uint8_t value0,
                                     uint8_t value1,
                                     uint8_t value2,
                                     uint8_t value3) {
        return static_cast<uint8_t>((static_cast<int>(value0) + static_cast<int>(value1) +
    ...
    template <typename ColorType> inline
    const ColorType Average4ColorsFast(ColorType const& color0,
                                       ColorType const& color1,
                                       ColorType const& color2,
                                       ColorType const& color3);
    
    // Specialized version for Rgb888 colors.
    template <> inline
    const Rgb888 Average4ColorsFast(Rgb888 const& color0,
                                    Rgb888 const& color1,
                                    Rgb888 const& color2,
                                    Rgb888 const& color3) {
    ...
    template <> inline
    const Rgba8888 Average4ColorsFast(Rgba8888 const& color0,
                                      Rgba8888 const& color1,
                                      Rgba8888 const& color2,
                                      Rgba8888 const& color3) {
    ...
    static ColorType ComputeAveragePixel2x2(const ColorType pixels[4][4], int row, int col) {
        return Average4ColorsFast<ColorType>(pixels[row][col], pixels[row][col + 1],
                                             pixels[row + 1][col], pixels[row + 1][col + 1]);
    }
    
    // Computes the approximate luminance of an RGB color.
    inline int ComputeLuminanceFast(RgbInt const& c) {
    ...
    inline int ComputeSquaredLuminanceDistanceFast(RgbInt const& c0, RgbInt const& c1) {
    ...
    inline int ComputeDifferenceLuminanceFast(RgbInt const& c0, RgbInt const& c1) {
        const RgbInt cdiff(std::abs(c0.r - c1.r), std::abs(c0.g - c1.g), std::abs(c0.b - c1.b));
    ...
    template <typename ColorType> inline
    ColorType* GetMutableColorInImageBuffer(ColorType* image_buffer,
                                            int bytes_per_row,
                                            int row, int column) {
        uint8_t* row_start = reinterpret_cast<uint8_t*>(image_buffer) + (row * bytes_per_row);
    ...
    // corresponding to a given row and column in an image buffer.
    template <typename ColorType> inline
    const ColorType* GetColorInImageBuffer(const ColorType* image_buffer,
                                           int bytes_per_row,
                                           int row, int column) {
        const uint8_t* row_start = reinterpret_cast<const uint8_t*>(image_buffer) + (row * bytes_per_row);
    ...
    template <typename ColorType> inline
    void StoreColorsInBuffer(const ColorType block_pixels[4][4],
                             int image_width,
                             int num_rows_in_block, int num_cols_in_block,
                             int row, int col,
                             ColorType* image_buffer) {
        for (int y = 0; y < num_rows_in_block; ++y) {
            ColorType* row_pixels = GetMutableColorInImageBuffer(image_buffer, image_width, row + y, col);
    ...
    std::ostream& operator<<(std::ostream& out, RgbColor<T> const& c) {
        out << "[" << c.r << ", " << c.g << ", " << c.b << "]";
        return out;
    }
    
    template <typename T> inline
    std::ostream& operator<<(std::ostream& out, RgbaColor<T> const& c) {
        out << "[" << c.r << ", " << c.g << ", " << c.b << ", " << c.a << "]";
        return out;
    }
