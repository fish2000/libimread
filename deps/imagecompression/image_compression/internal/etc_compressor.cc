...
namespace imagecompression {
    ...
    class EtcHelper {
        ...
        public:
            ...
            static const RgbInt AddCodebookModifier(const RgbInt color, int codeword, int pixel_index) {
            ...
            static int GetPixelIndex(int y, int x, uint32_t pixel_indices) {
                const int p = GetPixelOrderIndex(y, x);
                return GetBits(pixel_indices, p, 1) | (GetBits(pixel_indices, p + 16, 1) << 1);
            }
            
            // Stores the 2-bit code index for a pixel in the given 32-bit
            // encoding word. The x and y of the pixel are passed in.
            static void StorePixelIndex(int y, int x, int code, uint32_t* pixel_indices) {
                DCHECK_GE(code, 0);
                DCHECK_LT(code, 4);
                const int p = GetPixelOrderIndex(y, x);
    ...
    #else
                return (static_cast<uint64_t>(hi_word) << 32) | lo_word;
    #endif
            }
        ...
        class Etc1BlockDecoder {
            public:
                ...
                explicit Etc1BlockDecoder(uint64_t bits);
                ...
                const Rgb888 GetPixel(int y, int x) const {
                    ...
                    return ToRgb888(
                        EtcHelper::AddCodebookModifier(color, codewords_[codeword_index], pixel_index));
                }
    ...
    Etc1BlockDecoder::Etc1BlockDecoder(uint64_t bits) {
        ...
        if (diff_bit_) {
            ...
            base_color1_ =
                RgbInt(Extend4Bit(GetBits(hi_word, 28, 4)),
                       Extend4Bit(GetBits(hi_word, 20, 4)),
                       Extend4Bit(GetBits(hi_word, 12, 4)));
            base_color2_ =
                RgbInt(Extend4Bit(GetBits(hi_word, 24, 4)),
    ...
    static void DecodeBlock(uint64_t block, Rgb888 decoded_pixels[4][4]) {
    ...
    // array of RGB pixels.
    static const RgbInt ComputeAverageColor(Pixel4x4 const& pixel4x4,
                                            Etc1Subblock const& subblock) {
        // There should be 8 pixels in the subblock.
        DCHECK_EQ((subblock.x_max - subblock.x_min + 1) * (subblock.y_max - subblock.y_min + 1), 8);
    ...
    static void StoreNormalModeColors(const RgbInt color1, const RgbInt color2, uint32_t* hi_word) {
    ...
    // given high-order block word.
    static void StoreDiffModeColors(const RgbInt color1, const RgbInt color_difference,
                                    uint32_t* hi_word) {
    ...
    static uint32_t ComputeColorError(const RgbInt c0, const RgbInt c1) {
    ...
    static uint32_t ComputeCodewordError(Pixel4x4 const& pixel4x4,
                                         Etc1Subblock const& subblock,
                                         int codeword, const RgbInt color,
                                         uint32_t* pixel_indices) {
        uint32_t cumulative_error = 0;
        *pixel_indices            = 0;
        ...
        for (int y = subblock.y_min; y <= subblock.y_max; ++y) {
            for (int x = subblock.x_min; x <= subblock.x_max; ++x) {
                ...
                int          best_index     = 0;
                uint32_t     smallest_error = ComputeColorError(target_color, c0);
                uint32_t     new_error      = ComputeColorError(target_color, c1);
    ...
    static int FindBestCodeword(Pixel4x4 const& pixel4x4,
                                Etc1Subblock const& subblock,
                                const RgbInt color,
                                uint32_t* pixel_indices,
                                uint32_t* cumulative_error) {
        // Try each codeword and choose the one with the smallest error.
        int best_codeword = -1;
        *cumulative_error = kUInt32max;
        for (int i = 0; i < 8; ++i) {
            uint32_t       indices;
            const uint32_t error = ComputeCodewordError(pixel4x4, subblock, i, color, &indices);
    ...
    // in cumulative_error.
    static int FindCodewordHeuristic(Pixel4x4 const& pixel4x4,
                                     Etc1Subblock const& subblock,
                                     const RgbInt color, uint32_t* pixel_indices,
                                     uint32_t* cumulative_error) {
        ...
        *cumulative_error = ComputeCodewordError(pixel4x4, subblock, codeword, color, pixel_indices);
        return codeword;
    }   
    ...
    static uint64_t FindBestSubblockEncoding(Pixel4x4 const& pixel4x4, bool flip,
                                             EtcCompressor::CompressionStrategy strategy,
                                             uint32_t* cumulative_error) {
        const Etc1Subblock subblock1 = flip ? Etc1Subblock(0, 3, 0, 1) : Etc1Subblock(0, 1, 0, 3);
        const Etc1Subblock subblock2 = flip ? Etc1Subblock(0, 3, 2, 3) : Etc1Subblock(2, 3, 0, 3);
        ...
        const bool use_diff_mode = (difference.r >= -4 && difference.r <= 3 && difference.g >= -4 &&
                                    difference.g <= 3 && difference.b >= -4 && difference.b <= 3);
        ...
        uint32_t pixel_indices1 = 0, pixel_indices2 = 0;
        uint32_t error1, error2;
        int      codeword1;
        int      codeword2;
        if (strategy == EtcCompressor::kHeuristic) {
            codeword1 = FindCodewordHeuristic(pixel4x4, subblock1, decoded_color1, &pixel_indices1,
                                              &error1);
            codeword2 = FindCodewordHeuristic(pixel4x4, subblock2, decoded_color2, &pixel_indices2,
                                              &error2);
        } else {
            codeword1 = FindBestCodeword(pixel4x4, subblock1, decoded_color1, &pixel_indices1, &error1);
            codeword2 = FindBestCodeword(pixel4x4, subblock2, decoded_color2, &pixel_indices2, &error2);
        }
    ...
    uint64_t EncodeEtc1Block(Pixel4x4 const& pixel4x4,
        ...
        switch (strategy) {
            ...
            default: {
                ...
                const uint64_t block_lr =
                    FindBestSubblockEncoding(pixel4x4, false, strategy, &error_lr);
                const uint64_t block_tb =
                    FindBestSubblockEncoding(pixel4x4, true, strategy, &error_tb);
    ...
    static const uint64_t CreateSolidBlock(Rgb888 const& color) {
        uint32_t hi_word = 0;
    ...
    struct EtcDecode {
        void operator()(uint64_t block, bool swap_red_and_blue, Rgb888 decoded_pixels[4][4]) {
            DCHECK(!swap_red_and_blue);
    ...
    class EtcGetColumnPadBlock {
        public:
            explicit EtcGetColumnPadBlock(EtcCompressor::CompressionStrategy strategy)
                : strategy_(strategy) {}
            const uint64_t operator()(uint64_t const& last_column_block) {
    ...
    struct EtcGetRowPadBlock {
        public:
            explicit EtcGetRowPadBlock(EtcCompressor::CompressionStrategy strategy)
                : strategy_(strategy) {}
            const uint64_t operator()(uint64_t const& last_row_block) {
                // TODO(user): Optimize this by operating directly on encoded blocks?
                // Decode the block to get colors from it.
    ...
    struct EtcGetCornerPadBlock {
        const uint64_t operator()(uint64_t const& last_block) {
    ...
    bool EtcCompressor::IsValidCompressedImage(CompressedImage const& image) {
        const CompressedImage::Metadata& metadata = image.GetMetadata();
        return metadata.format == CompressedImage::kRGB && metadata.compressor_name == "etc" &&
               metadata.uncompressed_height > 0 && metadata.uncompressed_width > 0 &&
    ...
    size_t EtcCompressor::ComputeCompressedDataSize(CompressedImage::Format format,
        ...
        if (format == CompressedImage::kRGB) {
            ...
            return std::max(1U, num_block_rows) * std::max(1U, num_block_cols) * sizeof(uint64_t);
        }
        return 0;
    }
    
    bool EtcCompressor::Compress(CompressedImage::Format format, uint32_t height, uint32_t width,
                                 uint32_t padding_bytes_per_row, const uint8_t* buffer,
                                 CompressedImage* image) {
        if (!buffer || !image || height == 0 || width == 0 || format != CompressedImage::kRGB) {
            return false;
        }
        return Compressor4x4Helper::Compress<uint64_t, Rgb888>(EtcEncode(compression_strategy_),
                                                              "etc", format,
                                                                     height, width,
    ...
    bool EtcCompressor::Decompress(CompressedImage const& image,
                                   std::vector<uint8_t>* decompressed_buffer) {
        if (!IsValidCompressedImage(image) || !decompressed_buffer) {
            return false;
        }
        return Compressor4x4Helper::Decompress<uint64_t, Rgb888>(EtcDecode(),
                                                                 image,
                                                                 decompressed_buffer);
    }
    
    bool EtcCompressor::Downsample(CompressedImage const& image,
                                   CompressedImage* downsampled_image) {
        if (!IsValidCompressedImage(image) || !downsampled_image) {
            return false;
        }
        
        return Compressor4x4Helper::Downsample<uint64_t, Rgb888>(EtcEncode(compression_strategy_),
                                                                 EtcDecode(),
                                                                 image,
                                                                 downsampled_image);
    }
    
    bool EtcCompressor::Pad(CompressedImage const& image,
                            uint32_t padded_height, uint32_t padded_width,
                            CompressedImage* padded_image) {
        if (!IsValidCompressedImage(image) || !padded_image) {
            return false;
        }
        return Compressor4x4Helper::Pad<uint64_t>(EtcGetColumnPadBlock(compression_strategy_),
                                                  EtcGetRowPadBlock(compression_strategy_),
                                                  EtcGetCornerPadBlock(), image,
    ...
    bool EtcCompressor::CompressAndPad(CompressedImage::Format format,
        ...
        return Compressor4x4Helper::CompressAndPad<uint64_t, Rgb888>(EtcEncode(compression_strategy_),
                                                                    "etc", format,
                                                                           height, width,
                                                                           padded_height, padded_width,
                                                                           padding_bytes_per_row,
    ...
    bool EtcCompressor::CreateSolidImage(CompressedImage::Format format,
        ...
        return Compressor4x4Helper::CreateSolidImage<uint64_t>("etc", format,
                                                                      height,
                                                                      width, CreateSolidBlock(
                                                                                       Rgb888(color[0],
                                                                                              color[1],
                                                                                              color[2])), image);
    }
    
    bool EtcCompressor::CopySubimage(const CompressedImage& image, uint32_t start_row,
                                     uint32_t start_column, uint32_t height, uint32_t width,
                                     CompressedImage* subimage) {
        if (!IsValidCompressedImage(image) || !subimage) {
            return false;
        }
        return Compressor4x4Helper::CopySubimage<uint64_t>(image,
