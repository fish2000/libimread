...
namespace image_codec_compression {
    ...
    class Compressor4x4Helper {
        ...
        public:
            ...
            // dimension.
            static inline uint32_t GetNumBlocks(uint32_t num_pixels) { return (num_pixels + 3) / 4; }
            ...
            template <typename BlockType, typename ColorType, typename DecodeFunctor> inline
            static bool Decompress(DecodeFunctor decode,
                                   CompressedImage const& image,
            ...
            // dimension.
            template <typename BlockType, typename ColorType, typename EncodeFunctor,
                      typename DecodeFunctor> inline
            static bool Downsample(EncodeFunctor encode, DecodeFunctor decode,
                                   CompressedImage const& image,
                                   CompressedImage* downsampled_image);
            
            // Pads an image.
            template <typename BlockType, typename ColumnPadBlockFunctor, typename RowPadBlockFunctor,
                      typename CornerPadBlockFunctor> inline
            static bool Pad(ColumnPadBlockFunctor get_column_pad_block,
            ...
            template <typename BlockType, typename ColorType, typename EncodeFunctor> inline
            static bool CompressAndPad(EncodeFunctor encode,
                                       std::string const& compressor_name,
                                       CompressedImage::Format format,
                                       uint32_t height, uint32_t width,
                                       uint32_t padded_height, uint32_t padded_width,
            ...
            template <typename BlockType> inline
            static bool CreateSolidImage(std::string const& compressor_name,
                                         CompressedImage::Format format,
                                         uint32_t height, uint32_t width,
            ...
            template <typename BlockType> inline
            static bool CopySubimage(CompressedImage const& image,
        ...
        private:
            ...
            template <typename BlockType, typename ColorType, typename EncodeFunctor,
                      typename DecodeFunctor> inline
            static const BlockType DownsampleBlocks2x2(EncodeFunctor encode,
                                                       DecodeFunctor decode,
                                                       const BlockType* blocks[2][2]);
            
            template <typename BlockType, typename ColorType, typename EncodeFunctor,
                      typename DecodeFunctor> inline
            static const BlockType DownsampleBlocks2x1(EncodeFunctor encode,
                                                       DecodeFunctor decode,
                                                       const BlockType* blocks[2]);
            
            template <typename BlockType, typename ColorType, typename EncodeFunctor,
                      typename DecodeFunctor> inline
            static const BlockType DownsampleBlocks1x2(EncodeFunctor encode,
                                                       DecodeFunctor decode,
                                                       const BlockType* blocks[2]);
            ...
            static bool SetUpCompressedImage(std::string const& compressor_name,
                                             size_t block_size,
                                             CompressedImage::Format format,
                                             uint32_t height, uint32_t width,
                                             uint32_t padding_bytes_per_row,
    ...
    template <typename BlockType, typename ColorType, typename EncodeFunctor>
    bool Compressor4x4Helper::Compress(EncodeFunctor encode,
                                       std::string const& compressor_name,
                                       CompressedImage::Format format,
                                       uint32_t height, uint32_t width,
                                       uint32_t padding_bytes_per_row,
        ...
        DCHECK(buffer);
        DCHECK(image);
        if (!SetUpCompressedImage(compressor_name, sizeof(BlockType), format, height, width,
                                  padding_bytes_per_row, image)) {
            return false;
        }
        
        const ColorType* uncompressed_pixels = reinterpret_cast<const ColorType*>(buffer);
        ...
        for (uint32_t row = 0; row < num_block_rows; ++row) {
            for (uint32_t col = 0; col < num_block_cols; ++col) {
                ...
                DCHECK_LT(static_cast<const void*>(block),
                          static_cast<const void*>(image->GetData() + image->GetDataSize()));
    ...
    bool Compressor4x4Helper::Decompress(DecodeFunctor decode,
        ...
        decompressed_buffer->resize(metadata.uncompressed_height * metadata.uncompressed_width *
                                    sizeof(ColorType));
        
        const BlockType* block         = reinterpret_cast<const BlockType*>(image.GetData());
        ColorType* decompressed_pixels = reinterpret_cast<ColorType*>(&decompressed_buffer->at(0));
        ...
        const int  bytes_per_row = width * sizeof(decompressed_pixels[0]) + metadata.padding_bytes_per_row;
        ...
        for (int block_row = 0; block_row < num_block_rows; ++block_row) {
            for (int block_col = 0; block_col < num_block_cols; ++block_col) {
                ...
                if (rows_inside_image > 0 && cols_inside_image > 0)
                    StoreColorsInBuffer<ColorType>(block_pixels, bytes_per_row, rows_inside_image,
                                                   cols_inside_image, image_row, image_col,
                                                   decompressed_pixels);
            }
        }
        return true;
    ...
    template <typename BlockType, typename ColorType, typename EncodeFunctor,
              typename DecodeFunctor>
    bool Compressor4x4Helper::Downsample(EncodeFunctor encode,
                                         DecodeFunctor decode,
                                         CompressedImage const& image,
                                         CompressedImage* downsampled_image) {
        ...
        const uint32_t orig_height        = metadata.uncompressed_height;
        const uint32_t orig_width         = metadata.uncompressed_width;
        const uint32_t downsampled_height = (orig_height + 1) / 2;
        const uint32_t downsampled_width  = (orig_width + 1) / 2;
        if (!SetUpCompressedImage(metadata.compressor_name, sizeof(BlockType), metadata.format,
                                  downsampled_height, downsampled_width, 0, downsampled_image)) {
            return false;
        }
        
        const BlockType* orig_blocks = reinterpret_cast<const BlockType*>(image.GetData());
        BlockType*       downsampled_block = reinterpret_cast<BlockType*>(downsampled_image->GetMutableData());
        ...
        if (num_orig_block_rows > 1 && num_orig_block_cols > 1) {
            ...
            for (int row = 0; row < num_downsampled_block_rows; ++row) {
                for (int col = 0; col < num_downsampled_block_cols; ++col) {
                    ...
                    *downsampled_block++ = DownsampleBlocks2x2<BlockType,
                                                               ColorType,
                                                               EncodeFunctor,
                                                               DecodeFunctor>(encode, decode, blocks_to_downsample);
                }
            }
            ...
            for (int row = 0; row < num_downsampled_block_rows; ++row) {
                ...
                *downsampled_block++ = DownsampleBlocks2x1<BlockType,
                                                           ColorType,
                                                           EncodeFunctor,
                                                           DecodeFunctor>(encode, decode, blocks_to_downsample);
            }
            ...
            for (int col = 0; col < num_downsampled_block_cols; ++col) {
                ...
                *downsampled_block++ = DownsampleBlocks1x2<BlockType,
                                                           ColorType,
                                                           EncodeFunctor,
                                                           DecodeFunctor>(encode, decode, blocks_to_downsample);
            }
            ...
            for (int row = 0; row < 2; ++row) {
                for (int col = 0; col < 2; ++col) {
                    StoreDownsampledPixels4x4<ColorType>(decoded_pixels, 2 * row, 2 * col, &pixel4x4);
                }
            }
    ...
    template <typename BlockType, typename ColumnPadBlockFunctor, typename RowPadBlockFunctor,
              typename CornerPadBlockFunctor>
    bool Compressor4x4Helper::Pad(ColumnPadBlockFunctor  get_column_pad_block,
        ...
        if (!SetUpCompressedImage(metadata.compressor_name, sizeof(BlockType), metadata.format,
                                  padded_height, padded_width, 0, padded_image)) {
            return false;
        }
        ...
        const BlockType* orig_block_start = reinterpret_cast<const BlockType*>(image.GetData());
        BlockType*       padded_block_start = reinterpret_cast<BlockType*>(padded_image->GetMutableData());
        ...
        if (num_orig_block_rows < num_padded_block_rows) {
            ...
            const BlockType* last_block_row = orig_block_start + (num_orig_block_rows - 1) * num_orig_block_cols;
            ...
            if (num_orig_block_cols < num_padded_block_cols) {
                last_padded_row_blocks.insert(
                    last_padded_row_blocks.end(), num_padded_block_cols - num_orig_block_cols,
                    get_corner_pad_block(*(last_block_row + num_orig_block_cols - 1)));
            }
            // Copy the row into the padded buffer.
            BlockType* padded_block_row = padded_block_start + num_orig_block_rows * num_padded_block_cols;
            ...
            for (int row = num_orig_block_rows; row < num_padded_block_rows; ++row) {
                memcpy(padded_block_row, &last_padded_row_blocks[0], num_padded_row_bytes);
    ...
    template <typename BlockType, typename ColorType, typename EncodeFunctor>
    bool Compressor4x4Helper::CompressAndPad(EncodeFunctor           encode,
                                             std::string const&      compressor_name,
                                             CompressedImage::Format format,
                                             uint32_t height, uint32_t width,
                                             uint32_t padded_height, uint32_t padded_width,
        ...
        DCHECK(buffer);
        DCHECK(padded_image);
        const uint32_t final_height = std::max(height, padded_height);
        const uint32_t final_width  = std::max(width, padded_width);
        if (!SetUpCompressedImage(compressor_name, sizeof(BlockType), format, final_height,
                                  final_width, padding_bytes_per_row, padded_image)) {
            return false;
        }
        const ColorType* uncompressed_pixels = reinterpret_cast<const ColorType*>(buffer);
        BlockType*       block = reinterpret_cast<BlockType*>(padded_image->GetMutableData());
        const bool       swap_red_and_blue = NeedsRedAndBlueSwapped(format);
        const uint32_t num_block_rows = GetNumBlocks(final_height);
        const uint32_t num_block_cols = GetNumBlocks(final_width);
        DCHECK_GT(num_block_rows, 0);
        DCHECK_GT(num_block_cols, 0);
        for (uint32_t row = 0; row < num_block_rows; ++row) {
            for (uint32_t col = 0; col < num_block_cols; ++col) {
    ...
    bool Compressor4x4Helper::CreateSolidImage(std::string const& compressor_name,
                                               CompressedImage::Format format,
                                               uint32_t height, uint32_t width,
                                               BlockType const& block,
                                               CompressedImage* image) {
        if (!SetUpCompressedImage(compressor_name, sizeof(BlockType), format, height, width, 0,
                                  image)) {
            return false;
        }
        const uint32_t num_block_rows = GetNumBlocks(height);
        const uint32_t num_block_cols = GetNumBlocks(width);
        const uint32_t num_blocks     = num_block_rows * num_block_cols;
        // Copy block repeatedly.
        BlockType* block_start = reinterpret_cast<BlockType*>(image->GetMutableData());
    ...
    template <typename BlockType>
    bool Compressor4x4Helper::CopySubimage(CompressedImage const& image,
                                           uint32_t start_row, uint32_t start_column,
                                           uint32_t height, uint32_t width,
                                           CompressedImage* subimage) {
        DCHECK(subimage);
        ...
        const CompressedImage::Metadata& metadata = image.GetMetadata();
        if (start_row % 4 != 0 || start_column % 4 != 0 || height % 4 != 0 || width % 4 != 0 ||
            start_row > metadata.compressed_height || start_column > metadata.compressed_width ||
            start_row + height > metadata.compressed_height ||
            start_column + width > metadata.compressed_width) {
            return false;
        }
        if (!SetUpCompressedImage(metadata.compressor_name, sizeof(BlockType), metadata.format,
                                  height, width, 0, subimage)) {
            return false;
        }
        const BlockType* orig_block_start = reinterpret_cast<const BlockType*>(image.GetData());
        BlockType* subimage_block_start = reinterpret_cast<BlockType*>(subimage->GetMutableData());
        const int num_orig_block_cols     = GetNumBlocks(metadata.compressed_width);
        ...
        const BlockType* orig_block = orig_block_start + orig_start_block_row * num_orig_block_cols + orig_start_block_col;
        BlockType* subimage_block = subimage_block_start;
        for (int row = 0; row < num_subimage_block_rows; ++row) {
            memcpy(subimage_block, orig_block, num_subimage_block_cols * sizeof(*subimage_block));
            orig_block += num_orig_block_cols;
    ...
    template <typename BlockType, typename ColorType, typename EncodeFunctor,
              typename DecodeFunctor>
    const BlockType Compressor4x4Helper::DownsampleBlocks2x2(EncodeFunctor encode,
                                                             DecodeFunctor decode,
                                                             const BlockType* blocks[2][2]) {
        ColorType decoded_pixels[4][4];
        Pixel4x4  pixel4x4;
    ...
    template <typename BlockType, typename ColorType, typename EncodeFunctor,
              typename DecodeFunctor>
    const BlockType Compressor4x4Helper::DownsampleBlocks2x1(EncodeFunctor encode,
                                                             DecodeFunctor decode,
                                                             const BlockType* blocks[2]) {
        ColorType decoded_pixels[4][4];
        Pixel4x4  pixel4x4;
    ...
    template <typename BlockType, typename ColorType, typename EncodeFunctor,
              typename DecodeFunctor>
    const BlockType Compressor4x4Helper::DownsampleBlocks1x2(EncodeFunctor encode,
                                                             DecodeFunctor decode,
                                                             const BlockType* blocks[2]) {
        ColorType decoded_pixels[4][4];
        Pixel4x4  pixel4x4;
