...
namespace image_codec_compression {
    ...
    class Pixel4x4 {
        ...
        public:
            ...
            Pixel4x4(const ColorType* pixels,
                     uint32_t height, uint32_t width,
                     uint32_t padding_bytes_per_row,
                     int row, int column) {
                const uint32_t bytes_per_row = width * sizeof(pixels[0]) + padding_bytes_per_row;
                
                // In the common case where the 4x4 lies completely within the
                // image, do all the construction work here.
                if (static_cast<int32_t>(height) - row >= 4 &&
                    static_cast<int32_t>(width) - column >= 4) {
                    for (int y = 0; y < 4; ++y) {
                        const ColorType* row_pixels = GetImagePixel(pixels, bytes_per_row, row + y, column);
                        for (int x = 0; x < 4; ++x) {
                    ...
                    ConstructOutsideImage<ColorType>(pixels, height, width, bytes_per_row, row, column);
                }
            }
            
            // Stores a pixel color at the given row/column location.
            template <typename ColorType> inline
            void SetPixel(int row, int column, ColorType const& color) {
            ...
            template <typename ColorType> inline
            void SetAlpha(int row, int column, ColorType const& color) {}
            ...
            // original.
            inline bool has_one_pixel() const { return has_one_pixel_; }
            
            // Returns the color of a pixel.
            inline RgbInt const& GetPixel(int row, int column) const {
                DCHECK_GE(row, 0);
                DCHECK_GE(column, 0);
                DCHECK_LT(row, 4);
                DCHECK_LT(column, 4);
        ...
        private:
            ...
            template <typename ColorType> inline
            static const ColorType* GetImagePixel(const ColorType* pixels, int bytes_per_row,
                                                  int row, int column) {
                const uint8_t* row_start = reinterpret_cast<const uint8_t*>(pixels) + (row * bytes_per_row);
    ...
    template <> inline
    void Pixel4x4::SetAlpha(int row, int column, Rgba8888 const& color) {
    ...
    template <typename ColorType> inline
    void StoreDownsampledPixels4x4(const ColorType pixels[4][4],
