...
namespace imagecompression {
    ...
    struct Dxt1Block {
        ...
        Dxt1Block(const Rgb565& color0, const Rgb565& color1, const uint8_t bits[4]) {
            uint16_t c0 = ToUInt16(color0);
            uint16_t c1 = ToUInt16(color1);
            c0_lo       = c0 & 0xff;
            c0_hi       = c0 >> 8;
            c1_lo       = c1 & 0xff;
            c1_hi       = c1 >> 8;
    ...
    struct Dxt5Block {
        ...
        Dxt5Block(const Dxt1Block& block, uint8_t alpha) {
            dxt1_block = block;
            alpha1 = alpha0 = alpha;
            for (int i = 0; i < 6; ++i) {
                alpha_bits[i] = 0;
        ...
        // Constructor for a general block.
        Dxt5Block(const Dxt1Block& block, uint8_t base_alphas[2], const uint8_t bits[6]) {
            dxt1_block = block;
    ...
    class Dxt5AlphaBits {
        ...
        public:
            ...
            explicit Dxt5AlphaBits(const uint8_t bytes[6]) {
                int cur_index = 0;
                for (int i = 0; i < 6; ++i) {
                    uint8_t cur_byte = bytes[i];
                    for (int j = 0; j < 8; ++j) {
            ...
            void GetBytes(uint8_t bytes[6]) const {
                int cur_index = 0;
                for (int i = 0; i < 6; ++i) {
                    uint8_t cur_byte = 0;
                    for (int j = 0; j < 8; ++j) {
    ...
    static void DecodeColors(Dxt1Block const& block,
                             Rgb888 colors[4],
                             bool swap_red_and_blue,
                             bool always_4_color_case) {
        const uint16_t color0_as_uint16 = block.c0_lo + block.c0_hi * 256;
        const uint16_t color1_as_uint16 = block.c1_lo + block.c1_hi * 256;
    ...
    static void DecodeAlphaValues(Dxt5Block const& dxt5_block, uint8_t alpha[8]) {
        const uint8_t alpha0 = dxt5_block.alpha0;
        const uint8_t alpha1 = dxt5_block.alpha1;
        alpha[0] = alpha0;
        alpha[1] = alpha1;
        if (alpha0 > alpha1) {
    ...
    void DecodeDxt1Block(Dxt1Block const& dxt1_block,
        ...
        for (int y = 0; y < 4; ++y) {
            const uint8_t bits = dxt1_block.color_bits[y];
            for (int x = 0; x < 4; ++x) {
                uint8_t code         = (bits >> (2 * x)) & 3;
    ...
    static void DecodeDxt5Block(Dxt5Block const& block,
                                bool swap_red_and_blue,
        ...
        for (int y = 0; y < 4; ++y) {
            const uint8_t color_bits = block.dxt1_block.color_bits[y];
            for (int x = 0; x < 4; ++x) {
                const uint8_t color_code = (color_bits >> (2 * x)) & 3;
                const uint8_t alpha_code = alpha_bits.GetCode(cur_alpha_index);
    ...
    static size_t GetBlockSize(CompressedImage::Format format) {
        return GetNumFormatComponents(format) == 3 ? sizeof(Dxt1Block) : sizeof(Dxt5Block);
    }
    ...
    static void ComputeBaseColors(Pixel4x4 const& pixel4x4,
                                  bool swap_red_and_blue,
        ...
        if (!pixel4x4.has_one_pixel()) {
            ...
            for (int y = 0; y < 4; ++y) {
                for (int x = 0; x < 4; ++x) {
                    RgbInt const& color = ToRgbOrBgrInt(pixel4x4.GetPixel(y, x), swap_red_and_blue);
    ...
    static void ComputeColorBits(Pixel4x4 const& pixel4x4,
                                 bool swap_red_and_blue,
                                 const RgbInt base_colors[2],
                                 uint8_t color_bits[4]) {
        ...
        for (int y = 0; y < 4; ++y) {
            color_bits[y] = 0;
            for (int x = 0; x < 4; ++x) {
                const RgbInt color = ToRgbOrBgrInt(pixel4x4.GetPixel(y, x), swap_red_and_blue);
                int          which_color = 0;
                int          smallest_luminance_error = ComputeSquaredLuminanceDistanceFast(color, test_colors[0]);
                for (int c = 1; c < 4; ++c) {
                    const int luminance_error = ComputeSquaredLuminanceDistanceFast(color, test_colors[c]);
    ...
    // the base_color.
    static void ComputeConstantColorBits(Pixel4x4 const& pixel4x4,
                                         bool swap_red_and_blue,
                                         bool always_4_color_case,
                                         RgbInt const& base_color,
                                         Rgb565* color0, Rgb565* color1,
                                         uint8_t color_bits[4]) {
        RgbInt use_color   = ToRgbOrBgrInt(base_color, swap_red_and_blue);
        int    which_color = GetBestDxtcConstColors(use_color, color0, color1, always_4_color_case);
        // Replicate the 2 bit which_color for all the color_bits.
        uint8_t which_color_byte = which_color | (which_color << 2);
    ...
    static void ComputeBaseAlphas(Pixel4x4 const& pixel4x4, uint8_t base_alphas[2]) {
        // Special case for single-color blocks.
        if (pixel4x4.has_one_pixel()) {
    ...
    static void ComputeAlphaBits(Pixel4x4 const& pixel4x4,
                                 const uint8_t base_alphas[2],
                                 uint8_t alpha_bits[6]) {
        // Special case for single-color blocks.
        if (pixel4x4.has_one_pixel()) {
            for (int i = 0; i < 6; ++i)
        ...
        for (int y = 0; y < 4; ++y) {
            for (int x = 0; x < 4; ++x) {
                const int alpha          = pixel4x4.GetAlpha(y, x);
                int       which_alpha    = 0;
                int       smallest_error = ComputeSquaredComponentDistance(alpha, test_alphas[0]);
                for (int a = 1; a < 8; ++a) {
                    const int error = ComputeSquaredComponentDistance(alpha, test_alphas[a]);
    ...
    static Dxt1Block EncodeDxt1Block(Pixel4x4 const& pixel4x4,
                                     bool swap_red_and_blue,
                                     bool always_4_color_case) {
        ...
        // Convert pixel colors to Rgb565 form and to uint16_t for comparisons.
        Rgb565         color0    = QuantizeToRgb565(base_colors[0]);
        Rgb565         color1    = QuantizeToRgb565(base_colors[1]);
        const uint16_t color0_16 = ToUInt16(color0);
        const uint16_t color1_16 = ToUInt16(color1);
    ...
    static inline void CopyDxt1Colors(Dxt1Block const& from_block, Dxt1Block* to_block) {
    ...
    // columns.
    static inline uint8_t CopyColumn3ColorBits(uint8_t row_bits) {
        // Get the 2 bits for column 3.
        const uint8_t col3_bit_code = (row_bits >> 6) & 3;
    ...
    struct DxtcEncode {
        const BlockType operator()(Pixel4x4 const& pixel4x4,
                                   bool swap_red_and_blue);
    };
    
    template <>
    const Dxt1Block DxtcEncode<Dxt1Block>::operator()(Pixel4x4 const& pixel4x4,
                                                      bool swap_red_and_blue) {
        return EncodeDxt1Block(pixel4x4, swap_red_and_blue, false);
    }
    
    template <>
    const Dxt5Block DxtcEncode<Dxt5Block>::operator()(Pixel4x4 const& pixel4x4,
                                                      bool swap_red_and_blue) {
    ...
    template <typename BlockType>
    struct DxtcDecode {
        void operator()(BlockType const& block,
                        bool swap_red_and_blue,
                        typename BlockType::ColorType decoded_pixels[4][4]);
    };
    
    template <>
    void DxtcDecode<Dxt1Block>::operator()(Dxt1Block const& block,
                                           bool swap_red_and_blue,
                                           Rgb888 decoded_pixels[4][4]) {
    ...
    template <>
    void DxtcDecode<Dxt5Block>::operator()(Dxt5Block const& block,
                                           bool swap_red_and_blue,
                                           Rgba8888 decoded_pixels[4][4]) {
    ...
    template <typename BlockType>
    struct DxtcGetColumnPadBlock {
        const BlockType operator()(BlockType const& last_column_block);
    };
    
    template <>
    const Dxt1Block DxtcGetColumnPadBlock<Dxt1Block>::operator()(Dxt1Block const& last_column_block) {
        ...
        for (int row = 0; row < 4; ++row) {
            pad_block.color_bits[row] = CopyColumn3ColorBits(last_column_block.color_bits[row]);
        }
        return pad_block;
    }
    
    template <>
    const Dxt5Block DxtcGetColumnPadBlock<Dxt5Block>::operator()(Dxt5Block const& last_column_block) {
        // This is essentially the same as the DXT1 version, with the
        // addition of dealing with the alpha bits.
        Dxt5Block pad_block;
        pad_block.dxt1_block = DxtcGetColumnPadBlock<Dxt1Block>()(last_column_block.dxt1_block);
    ...
    template <typename BlockType>
    struct DxtcGetRowPadBlock {
        const BlockType operator()(BlockType const& last_row_block);
    };
    
    template <>
    const Dxt1Block DxtcGetRowPadBlock<Dxt1Block>::operator()(Dxt1Block const& last_row_block) {
        // Do this the quick way by using the same colors and just modifying
        // the bits for the last 3 rows.
        Dxt1Block pad_block;
        CopyDxt1Colors(last_row_block, &pad_block);
        const uint8_t last_row_bits = last_row_block.color_bits[3];
        for (int row = 0; row < 4; ++row) {
    ...
    template <>
    const Dxt5Block DxtcGetRowPadBlock<Dxt5Block>::operator()(Dxt5Block const& last_row_block) {
        // This is essentially the same as the DXT1 version, with the
        // addition of dealing with the alpha bits.
        Dxt5Block pad_block;
        pad_block.dxt1_block = DxtcGetRowPadBlock<Dxt1Block>()(last_row_block.dxt1_block);
    ...
    template <typename BlockType>
    struct DxtcGetCornerPadBlock {
        const BlockType operator()(BlockType const& last_block);
    };

    template <>
    const Dxt1Block DxtcGetCornerPadBlock<Dxt1Block>::operator()(Dxt1Block const& last_block) {
        // Do this the quick way by using the same colors and just modifying
        // the bits for all 16 pixels.
        Dxt1Block pad_block;
        CopyDxt1Colors(last_block, &pad_block);
        const uint8_t last_bits = CopyColumn3ColorBits(last_block.color_bits[3]);
        for (int row = 0; row < 4; ++row) {
    ...
    template <>
    const Dxt5Block DxtcGetCornerPadBlock<Dxt5Block>::operator()(Dxt5Block const& last_block) {
        // This is essentially the same as the DXT1 version, with the
        // addition of dealing with the alpha bits.
        Dxt5Block pad_block;
        pad_block.dxt1_block = DxtcGetCornerPadBlock<Dxt1Block>()(last_block.dxt1_block);
        pad_block.alpha0     = last_block.alpha0;
        pad_block.alpha1     = last_block.alpha1;
        Dxt5AlphaBits bits(last_block.alpha_bits);
        uint8_t       corner_bits = bits.GetCode(15);
        for (int i = 0; i < 16; ++i) {
    ...
    bool DxtcCompressor::IsValidCompressedImage(CompressedImage const& image) {
        const CompressedImage::Metadata& metadata = image.GetMetadata();
        return metadata.compressor_name == "dxtc" && metadata.uncompressed_height > 0 &&
    ...
    size_t DxtcCompressor::ComputeCompressedDataSize(CompressedImage::Format format,
        ...
        const uint32_t num_block_rows = Compressor4x4Helper::GetNumBlocks(height);
        const uint32_t num_block_cols = Compressor4x4Helper::GetNumBlocks(width);
        return std::max(1U, num_block_rows) * std::max(1U, num_block_cols) * GetBlockSize(format);
    }
    ...
    bool DxtcCompressor::Compress(CompressedImage::Format format,
        ...
        if (!buffer || !image || height == 0 || width == 0) {
            ...
            return Compressor4x4Helper::Compress<Dxt1Block, Rgb888>(
                DxtcEncode<Dxt1Block>(), "dxtc", format, height, width, padding_bytes_per_row,
                buffer, image);
        } else {
            return Compressor4x4Helper::Compress<Dxt5Block, Rgba8888>(
                DxtcEncode<Dxt5Block>(), "dxtc", format, height, width, padding_bytes_per_row,
                buffer, image);
        }
    }
    
    bool DxtcCompressor::Decompress(CompressedImage const& image,
                                    std::vector<uint8_t>* decompressed_buffer) {
        if (!IsValidCompressedImage(image) || !decompressed_buffer) {
            return false;
        } else if (GetNumFormatComponents(image.GetMetadata().format) == 3) {
            return Compressor4x4Helper::Decompress<Dxt1Block, Rgb888>(DxtcDecode<Dxt1Block>(),
                                                                      image, decompressed_buffer);
        } else {
            return Compressor4x4Helper::Decompress<Dxt5Block, Rgba8888>(DxtcDecode<Dxt5Block>(),
                                                                        image, decompressed_buffer);
        }
    }
    
    bool DxtcCompressor::Downsample(CompressedImage const& image,
        ...
        if (!IsValidCompressedImage(image) || !downsampled_image) {
            ...
            return Compressor4x4Helper::Downsample<Dxt1Block, Rgb888>(
                DxtcEncode<Dxt1Block>(), DxtcDecode<Dxt1Block>(), image, downsampled_image);
        } else {
            return Compressor4x4Helper::Downsample<Dxt5Block, Rgba8888>(
                DxtcEncode<Dxt5Block>(), DxtcDecode<Dxt5Block>(), image, downsampled_image);
        }
    }

    bool DxtcCompressor::Pad(CompressedImage const& image,
                             uint32_t padded_height, uint32_t padded_width,
                             CompressedImage* padded_image) {
        if (!IsValidCompressedImage(image) || !padded_image) {
            return false;
        } else if (GetNumFormatComponents(image.GetMetadata().format) == 3) {
            return Compressor4x4Helper::Pad<Dxt1Block>(DxtcGetColumnPadBlock<Dxt1Block>(),
                                                       DxtcGetRowPadBlock<Dxt1Block>(),
                                                       DxtcGetCornerPadBlock<Dxt1Block>(), image,
                                                       padded_height, padded_width, padded_image);
        } else {
            return Compressor4x4Helper::Pad<Dxt5Block>(DxtcGetColumnPadBlock<Dxt5Block>(),
                                                       DxtcGetRowPadBlock<Dxt5Block>(),
                                                       DxtcGetCornerPadBlock<Dxt5Block>(), image,
                                                       padded_height, padded_width, padded_image);
        }
    }
    ...
    bool DxtcCompressor::CompressAndPad(CompressedImage::Format format,
        ...
        if (!buffer || !padded_image || height == 0 || width == 0) {
            ...
            return Compressor4x4Helper::CompressAndPad<Dxt1Block, Rgb888>(
                DxtcEncode<Dxt1Block>(), "dxtc", format, height, width, padded_height, padded_width,
                padding_bytes_per_row, buffer, padded_image);
        } else {
            return Compressor4x4Helper::CompressAndPad<Dxt5Block, Rgba8888>(
                DxtcEncode<Dxt5Block>(), "dxtc", format, height, width, padded_height, padded_width,
                padding_bytes_per_row, buffer, padded_image);
        }
    }
    ...
    bool DxtcCompressor::CreateSolidImage(CompressedImage::Format format,
        ...
        if (!image) {
            ...
            return Compressor4x4Helper::CreateSolidImage<Dxt1Block>(
                ...
                Dxt1Block(QuantizeToRgb565(RgbInt(color[0], color[1], color[2]))), image);
        } else {
            return Compressor4x4Helper::CreateSolidImage<Dxt5Block>(
                "dxtc", format, height, width,
                Dxt5Block(Dxt1Block(QuantizeToRgb565(RgbInt(color[0], color[1], color[2]))),
                          color[3]),
                image);
        }
    }
    
    bool DxtcCompressor::CopySubimage(CompressedImage const& image,
        ...
        if (!IsValidCompressedImage(image) || !subimage) {
            ...
            return Compressor4x4Helper::CopySubimage<Dxt1Block>(image, start_row, start_column,
                                                                height, width, subimage);
        } else {
            return Compressor4x4Helper::CopySubimage<Dxt5Block>(image, start_row, start_column,
                                                                height, width, subimage);
        }
    }
