...
namespace imagecompression {
    ...
    class CompressedImage {
        ...
        public:
            ...
            struct Metadata {
                ...
                Metadata(Format format_in, std::string const& compressor_name_in,
                         uint32_t uncompressed_height_in, uint32_t uncompressed_width_in,
                         uint32_t compressed_height_in, uint32_t compressed_width_in,
                         uint32_t padding_bytes_per_row_in)
                    : format(format_in)
                ...
                uint32_t uncompressed_height;
                uint32_t uncompressed_width;
            ...
            CompressedImage(size_t data_size, uint8_t* external_data)
                : metadata_(kRGB, "", 0, 0, 0, 0, 0)
            ...
            void Duplicate(CompressedImage const& from) {
                ...
                if (&from != this || !OwnsData()) {
                    ...
                    const uint8_t* source_data = from.data_;
                    DCHECK(source_data);
    ...
    inline int GetNumFormatComponents(CompressedImage::Format format) {
        switch (format) {
            ...
            case CompressedImage::kBGR: return 3;
            case CompressedImage::kRGBA:
            case CompressedImage::kBGRA: return 4;
