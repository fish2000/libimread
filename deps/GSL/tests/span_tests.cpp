...
{
    {
        gsl::span<int> s = nullptr;
        CHECK((s.length() == 0 && s.data() == nullptr));
        
        gsl::span<const int> cs = nullptr;
        CHECK((cs.length() == 0 && cs.data() == nullptr));
    }
    
    {
        gsl::span<int, 0> s = nullptr;
        CHECK((s.length() == 0 && s.data() == nullptr));
        
        gsl::span<const int, 0> cs = nullptr;
        CHECK((cs.length() == 0 && cs.data() == nullptr));
    }
    
    {
#ifdef CONFIRM_COMPILATION_ERRORS
        gsl::span<int, 1> s = nullptr;
        CHECK((s.length() == 1 && s.data() == nullptr)); // explains why it can't compile
#endif
    }
...
{
    {
        gsl::span<int> s{ nullptr, static_cast<gsl::span<int>::index_type>(0) };
        CHECK((s.length() == 0 && s.data() == nullptr));
        
        gsl::span<const int> cs{ nullptr, static_cast<gsl::span<int>::index_type>(0) };
        CHECK((cs.length() == 0 && cs.data() == nullptr));
    }
    
    {
        gsl::span<int, 0> s{ nullptr, static_cast<gsl::span<int>::index_type>(0) };
        CHECK((s.length() == 0 && s.data() == nullptr));
        
        gsl::span<const int, 0> cs{ nullptr, static_cast<gsl::span<int>::index_type>(0) };
        CHECK((cs.length() == 0 && cs.data() == nullptr));
    }
    
    {
        auto workaround_macro = []() {
            gsl::span<int, 1> s{ nullptr, static_cast<gsl::span<int>::index_type>(0) };
        };
    ...
    {
        auto workaround_macro = []() {
            gsl::span<int> s{ nullptr, 1 };
        };
        CHECK_THROWS_AS(workaround_macro(), fail_fast);
        
        auto const_workaround_macro = []() {
            gsl::span<const int> cs{ nullptr, 1 };
        };
    ...
    {
        auto workaround_macro = []() {
            gsl::span<int, 0> s{ nullptr, 1 };
        };
        CHECK_THROWS_AS(workaround_macro(), fail_fast);
        
        auto const_workaround_macro = []() {
            gsl::span<const int, 0> s{ nullptr, 1 };
        };
        CHECK_THROWS_AS(const_workaround_macro(), fail_fast);
    }
...
TEST_CASE("from_pointer_length_constructor")
{
    ...
    {
        int* p = nullptr;
        gsl::span<int> s{ p, static_cast<gsl::span<int>::index_type>(0) };
        CHECK((s.length() == 0 && s.data() == nullptr));
    }
    ...
    {
        int* p = nullptr;
        auto s = gsl::make_span(p, static_cast<gsl::span<int>::index_type>(0));
        CHECK((s.length() == 0 && s.data() == nullptr));
    }
...
TEST_CASE("from_pointer_pointer_constructor")
{
    ...
    //{
    //    auto workaround_macro = [&]() { gsl::span<int> s{ &arr[1], &arr[0] }; };
    //    CHECK_THROWS_AS(workaround_macro(), fail_fast);
    //}
    ...
    //{
    //    int* p = nullptr;
    //    auto workaround_macro = [&]() { gsl::span<int> s{ &arr[0], p }; };
    //    CHECK_THROWS_AS(workaround_macro(), fail_fast);
    //}
    ...
    {
        auto s = gsl::make_span(&arr[0], &arr[0]);
        CHECK((s.length() == 0 && s.data() == &arr[0]));
    }
    
    {
        int* p = nullptr;
        auto s = gsl::make_span(p, p);
        CHECK((s.length() == 0 && s.data() == nullptr));
    }
}
...
TEST_CASE("from_array_constructor")
{
...
#ifdef CONFIRM_COMPILATION_ERRORS
    {
        gsl::span<int, 6> s{ arr };
    }
    
    {
        gsl::span<int, 0> s{ arr };
        CHECK((s.length() == 0 && s.data() == &arr[0]));
    }
    
    {
        gsl::span<int> s{ arr2d };
        CHECK((s.length() == 6 && s.data() == &arr2d[0][0]));
        CHECK((s[0] == 1 && s[5] == 6));
    }
    
    {
        gsl::span<int, 0> s{ arr2d };
        CHECK((s.length() == 0 && s.data() == &arr2d[0][0]));
    }
    
    {
        gsl::span<int, 6> s{ arr2d };
    }
#endif
    {
        gsl::span<int[3]> s{ &(arr2d[0]), 1 };
...
#ifdef CONFIRM_COMPILATION_ERRORS
    {
        gsl::span<int> s{ arr3d };
        CHECK((s.length() == 12 && s.data() == &arr3d[0][0][0]));
        CHECK((s[0] == 1 && s[11] == 12));
    }
    
    {
        gsl::span<int, 0> s{ arr3d };
        CHECK((s.length() == 0 && s.data() == &arr3d[0][0][0]));
    }
    
    {
        gsl::span<int, 11> s{ arr3d };
    }
    
    {
        gsl::span<int, 12> s{ arr3d };
        CHECK((s.length() == 12 && s.data() == &arr3d[0][0][0]));
        CHECK((s[0] == 1 && s[5] == 6));
    }
#endif
    ...
    {
        auto s = gsl::make_span(&(arr2d[0]), 1);
        CHECK((s.length() == 1 && s.data() == &arr2d[0]));
    }
    
    {
        auto s = gsl::make_span(&arr3d[0], 1);
        CHECK((s.length() == 1 && s.data() == &arr3d[0]));
    }
}
...
TEST_CASE("from_dynamic_array_constructor")
{
    ...
    {
        gsl::span<double> s(&arr[0][0][0], 10);
        CHECK((s.length() == 10 && s.data() == &arr[0][0][0]));
    }
    
    {
        auto s = gsl::make_span(&arr[0][0][0], 10);
        CHECK((s.length() == 10 && s.data() == &arr[0][0][0]));
    }
...
TEST_CASE("from_std_array_constructor")
{
    ...
    {
        gsl::span<int> s{ arr };
        CHECK((s.size() == gsl::narrow_cast<std::ptrdiff_t>(arr.size()) && s.data() == arr.data()));
        
        gsl::span<const int> cs{ arr };
        CHECK((cs.size() == gsl::narrow_cast<std::ptrdiff_t>(arr.size()) && cs.data() == arr.data()));
    }
    
    {
        gsl::span<int, 4> s{ arr };
        CHECK((s.size() == gsl::narrow_cast<std::ptrdiff_t>(arr.size()) && s.data() == arr.data()));
        
        gsl::span<const int, 4> cs{ arr };
        CHECK((cs.size() == gsl::narrow_cast<std::ptrdiff_t>(arr.size()) && cs.data() == arr.data()));
    }
    
#ifdef CONFIRM_COMPILATION_ERRORS
    {
        gsl::span<int, 2> s{ arr };
        CHECK((s.size() == 2 && s.data() == arr.data()));
        
        gsl::span<const int, 2> cs{ arr };
    ...
    
    {
        ...
        auto take_a_span = [](gsl::span<int> s) { static_cast<void>(s); };
    ...
    {
        auto get_an_array = []() -> std::array<int, 4> { return { 1, 2, 3, 4 }; };
        auto take_a_span = [](gsl::span<const int> s) { static_cast<void>(s); };
        // try to take a temporary std::array
        take_a_span(get_an_array());
    }
...
TEST_CASE("from_const_std_array_constructor")
{
    ...
    {
        gsl::span<const int> s{ arr };
        CHECK((s.size() == gsl::narrow_cast<std::ptrdiff_t>(arr.size()) && s.data() == arr.data()));
    }
    
    {
        gsl::span<const int, 4> s{ arr };
        CHECK((s.size() == gsl::narrow_cast<std::ptrdiff_t>(arr.size()) && s.data() == arr.data()));
    }
    
#ifdef CONFIRM_COMPILATION_ERRORS
    {
        gsl::span<const int, 2> s{ arr };
        CHECK((s.size() == 2 && s.data() == arr.data()));
    }
    
    {
        gsl::span<const int, 0> s{ arr };
        CHECK((s.size() == 0 && s.data() == arr.data()));
    }
    
    {
        gsl::span<const int, 5> s{ arr };
    }
#endif
...
TEST_CASE("from_unique_pointer_construction")
{
    ...
    
    {
        ...
        {
            gsl::span<int> s{ arr, 4 };
            CHECK((s.length() == 4 && s.data() == arr.get()));
            CHECK((s[0] == 1 && s[1] == 2));
        }
        
        {
            auto s = gsl::make_span(arr, 4);
            CHECK((s.length() == 4 && s.data() == arr.get()));
            CHECK((s[0] == 1 && s[1] == 2));
        }
    }
    
    {
        ...
        {
            gsl::span<int> s{ arr, 0 };
            CHECK((s.length() == 0 && s.data() == nullptr));
        }
        
        {
            auto s = gsl::make_span(arr, 0);
            CHECK((s.length() == 0 && s.data() == nullptr));
        }
    }
}
...
TEST_CASE("from_container_constructor")
{
    ...
    {
        gsl::span<int> s{ v };
        CHECK((s.size() == gsl::narrow_cast<std::ptrdiff_t>(v.size()) && s.data() == v.data()));
        
        gsl::span<const int> cs{ v };
        CHECK((cs.size() == gsl::narrow_cast<std::ptrdiff_t>(v.size()) && cs.data() == v.data()));
    }
...
#ifdef CONFIRM_COMPILATION_ERRORS
        gsl::span<char> s{ str };
        CHECK((s.size() == gsl::narrow_cast<std::ptrdiff_t>(str.size()) && s.data() == str.data()));
#endif
        gsl::span<const char> cs{ str };
        CHECK((cs.size() == gsl::narrow_cast<std::ptrdiff_t>(str.size()) && cs.data() == str.data()));
    }
    
    {
#ifdef CONFIRM_COMPILATION_ERRORS
        gsl::span<char> s{ cstr };
#endif
        gsl::span<const char> cs{ cstr };
        CHECK((cs.size() == gsl::narrow_cast<std::ptrdiff_t>(cstr.size()) &&
              cs.data() == cstr.data()));
    }
    
    {
        ...
        auto use_span = [](gsl::span<int> s) { static_cast<void>(s); };
    ...
    {
        auto get_temp_vector = []() -> std::vector<int> { return {}; };
        auto use_span = [](gsl::span<const int> s) { static_cast<void>(s); };
        use_span(get_temp_vector());
    }
    
    {
        ...
        auto use_span = [](gsl::span<char> s) { static_cast<void>(s); };
    ...
    {
        auto get_temp_string = []() -> std::string { return {}; };
        auto use_span = [](gsl::span<const char> s) { static_cast<void>(s); };
        use_span(get_temp_string());
    }
    ...
    {
        auto s = gsl::make_span(v);
        CHECK((s.size() == gsl::narrow_cast<std::ptrdiff_t>(v.size()) && s.data() == v.data()));
        
        auto cs = gsl::make_span(cv);
        CHECK((cs.size() == gsl::narrow_cast<std::ptrdiff_t>(cv.size()) && cs.data() == cv.data()));
    }
}
...
{
    {
        gsl::span<DerivedClass> avd;
        gsl::span<const DerivedClass> avcd = avd;
    ...
    {
        gsl::span<int> s;
        gsl::span<unsigned int> s2 = s;
        static_cast<void>(s2);
    }
    
    {
        gsl::span<int> s;
        gsl::span<const unsigned int> s2 = s;
        static_cast<void>(s2);
    }
...
TEST_CASE("copy_move_and_assignment")
{
    ...
    auto use_span = [&](gsl::span<const int> s) { CHECK((s.length() == 2 && s.data() == &arr[1])); };
...
{
    gsl::span<int>::const_iterator it1;
    gsl::span<int>::const_iterator it2;
    CHECK(it1 == it2);
}

TEST_CASE("iterator_conversions")
{
    gsl::span<int>::iterator badIt;
    gsl::span<int>::const_iterator badConstIt;
    ...
    gsl::span<int>::const_iterator cit2 = it;
    CHECK(cit2 == cit);
    
    gsl::span<int>::const_iterator cit3 = it + 4;
    CHECK(cit3 == s.cend());
}
...
TEST_CASE("iterator_comparisons")
{
    int a[] = { 1, 2, 3, 4 };
    {
        ...
        auto it2 = it + 1;
        gsl::span<int>::const_iterator cit = s.cbegin();
...
{
    {
        ...
        gsl::span<int>::const_iterator cit = s.cbegin();
        gsl::span<int>::const_iterator cit2 = std::cbegin(s);
        CHECK(cit == cit2);
...
{
    {
        gsl::span<int> s1 = nullptr;
        gsl::span<int> s2 = nullptr;
...
TEST_CASE("as_bytes")
{
    ...
    
    {
        ...
        CHECK(s.length() == 4);
        const gsl::span<const byte> bs = as_bytes(s);
...
TEST_CASE("as_writeable_bytes")
{
    ...
    
    {
        ...
        gsl::span<const int> s = a;
        CHECK(s.length() == 4);
        gsl::span<const byte> bs = as_writeable_bytes(s);
...
// initialization or assignment to static span that REDUCES size is NOT ok
    ...
    {
        const gsl::span<int, 2> s2 = { arr, 2 };
        static_cast<void>(s2);
    }
    {
        const gsl::span<int, 1> s1 = s4.first<1>();
        static_cast<void>(s1);
    }
...
#ifdef CONFIRM_COMPILATION_ERRORS
    {
        gsl::span<int, 4> s3 = arr2;
    }
    {
        gsl::span<int, 2> s2 = arr2;
        gsl::span<int, 4> s4a = s2;
    }
#endif
    {
        auto f = [&]() {
            gsl::span<int, 4> _s4 = { arr2, 2 };
    ...
    gsl::span<int> av = arr2;
    auto f = [&]() {
        gsl::span<int, 4> _s4 = av;
        static_cast<void>(_s4);
    };
    CHECK_THROWS_AS(f(), fail_fast);
}
...
TEST_CASE("interop_with_std_regex")
{
    ...
    std::match_results<gsl::span<char>::iterator> match;
...
{
    CHECK((std::is_default_constructible<gsl::span<int>>::value));
    CHECK((std::is_default_constructible<gsl::span<int, 0>>::value));
    CHECK((!std::is_default_constructible<gsl::span<int, 42>>::value));
}
