...
namespace im {
    ...
    namespace {
        ...
        boolean empty_output_buffer(j_compress_ptr cinfo) {
            jpeg_dst_adaptor *adaptor
                = reinterpret_cast<jpeg_dst_adaptor *>(cinfo->dest);
        ...
        void flush_output_buffer(j_compress_ptr cinfo) {
            jpeg_dst_adaptor *adaptor
                = reinterpret_cast<jpeg_dst_adaptor *>(cinfo->dest);
            adaptor->s->write_check(adaptor->buf, adaptor->mgr.next_output_byte
                                                  - adaptor->buf);
        }
    ...
    void JPEGFormat::write(Image *input, byte_sink *output,
                           const options_map &opts) {
        if (input->nbits() != 8)
            throw CannotWriteError("Image must be 8 bits for JPEG saving");
